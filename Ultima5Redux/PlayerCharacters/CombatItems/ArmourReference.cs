...
namespace Ultima5Redux.PlayerCharacters.CombatItems
{
    public class ArmourReference : CombatItemReference
    {
        ...
        {
            if (CombatItemReferences.EquipmentMatches(Enum.GetValues(typeof(HelmEnum)), ref equipment))
                return ArmourType.Helm;
            if (CombatItemReferences.EquipmentMatches(Enum.GetValues(typeof(ChestArmourEnum)), ref equipment))
                return ArmourType.ChestArmour;
            if (CombatItemReferences.EquipmentMatches(Enum.GetValues(typeof(RingEnum)), ref equipment))
                return ArmourType.Ring;
            if (CombatItemReferences.EquipmentMatches(Enum.GetValues(typeof(AmuletEnum)), ref equipment))
